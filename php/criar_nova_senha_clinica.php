...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $con->prepare("SELECT * FROM clinica WHERE token = ? AND token_expira > NOW()");
    ...
    if ($resultado->num_rows == 1) {
        ...
        $updateStmt = $con->prepare("UPDATE clinica SET password = ?, token = '', token_expira = NULL WHERE id = ?");
